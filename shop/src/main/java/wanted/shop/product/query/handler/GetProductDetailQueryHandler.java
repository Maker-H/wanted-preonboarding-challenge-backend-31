...
public class GetProductDetailQueryHandler {
    ...
    public GetProductResult handle(ProductId productId) {
        ...
        // 관계형 테이블 조회
        List<GetProductResult.CategoryInfo> categoryInfos = productCategoryRepository.findCategoryInfoByProductId(productId)
        ...
        List<GetProductResult.OptionGroupInfo> optionGroupInfos = ProductOptionFlatProjection.toResponse(optionFlatProjections);

        List<GetProductResult.TagInfo> tagInfos = productTagRepository.findTagInfoByProductId(productId)
